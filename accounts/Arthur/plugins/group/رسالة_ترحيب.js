...
const NovaUltra = { command: ['ترحيب'], description: 'تعيين رسالة الترحيب', elite: 'off', group: true, prv: false, lock: 'off' };
...
async function execute({ sock, msg, args }) {
    ...
    if (!value) return reply(sock, chatId,
        'ꕥ أرسل رسالة الترحيب بعد الأمر.\n> يمكنك استخدام {user} و {group} كمتغيرات.\n\n✐ مثال:\nترحيب أهلاً {user} في {group}!', msg);

    const d = loadF();
    const g = getGroup(d, chatId);
    g.welcome = value;
    saveF(d);
    await reply(sock, chatId, 'ꕥ تم تعيين رسالة الترحيب بنجاح.', msg);
}
