...
const NovaUltra = { command: ['خفض'], description: 'تخفيض مشرف لعضو عادي', elite: 'off', group: true, prv: false, lock: 'off' };
...
async function execute({ sock, msg, args }) {
    ...
    if (!user) return reply(sock, chatId, '《✧》 منشن أو رد على رسالة المشرف الذي تريد خفضه.', msg);

    const meta  = await sock.groupMetadata(chatId);
    const found = meta.participants.find(p => p.id.replace(/:\d+/,'') === user.replace(/:\d+/,''));
    if (!found?.admin) return sock.sendMessage(chatId, { text: `《✧》 @${user.split('@')[0]} ليس مشرفاً!`, mentions: [user] }, { quoted: msg });
    if (user === meta.owner) return reply(sock, chatId, '《✧》 لا يمكن خفض مالك المجموعة.', msg);

    await sock.groupParticipantsUpdate(chatId, [user], 'demote');
    await sock.sendMessage(chatId, { text: `✿ @${user.split('@')[0]} *تم خفضه* لعضو عادي.`, mentions: [user] }, { quoted: msg });
}
