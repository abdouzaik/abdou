...
const NovaUltra = { command: ['تحذيرات'], description: 'عرض تحذيرات عضو', elite: 'off', group: true, prv: false, lock: 'off' };
...
async function execute({ sock, msg, args }) {
    ...
    if (!user) return reply(sock, chatId, '《✧》 منشن أو رد على رسالة الشخص لعرض تحذيراته.', msg);
    ...
    await sock.sendMessage(chatId, {
        text: `✐ تحذيرات @${user.split('@')[0]}:\n> ✧ المجموع: \`${warns.length}/${limit}\`\n\n${list}`,
        mentions: [user]
    }, { quoted: msg });
}
