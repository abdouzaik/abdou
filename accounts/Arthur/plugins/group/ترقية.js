...
const NovaUltra = { command: ['رفع'], description: 'ترقية عضو لمشرف', elite: 'off', group: true, prv: false, lock: 'off' };
...
async function execute({ sock, msg, args }) {
    ...
    if (!user) return reply(sock, chatId, '《✧》 منشن أو رد على رسالة الشخص الذي تريد ترقيته.', msg);

    const meta = await sock.groupMetadata(chatId);
    const found = meta.participants.find(p => p.id.replace(/:\d+/,'') === user.replace(/:\d+/,''));
    if (found?.admin) return sock.sendMessage(chatId, { text: `《✧》 @${user.split('@')[0]} مشرف بالفعل!`, mentions: [user] }, { quoted: msg });

    await sock.groupParticipantsUpdate(chatId, [user], 'promote');
    await sock.sendMessage(chatId, { text: `✿ @${user.split('@')[0]} *تمت ترقيته* لمشرف!`, mentions: [user] }, { quoted: msg });
}
