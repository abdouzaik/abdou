...
const NovaUltra = {
    command: ['مستوى', 'lvl', 'level'],
    description: 'عرض مستوى وخبرة المستخدم',
...
async function execute({ sock, msg, args }) {
    ...
    const who       = mentioned[0] || quoted || senderJid;

    const { db, user } = getUser(who);
    if (!user.name && who !== senderJid)
    ...
    const { min, xp } = xpRange(user.level || 0);
    const progreso = (user.exp || 0) - min;
    const pct      = xp > 0 ? Math.floor((progreso / xp) * 100) : 0;

    const text = `*「✿」مستخدم* ◢ ${user.name || 'مستخدم'} ◤

❖ المستوى › *${user.level || 0}*
☆ الخبرة › *${(user.exp || 0).toLocaleString()}*
➨ التقدم › *${progreso} => ${xp}* _(${pct}%)_
✐ الترتيب › *#${rank}*
❒ الأوامر المنفذة › *${(user.usedcommands || 0).toLocaleString()}*`;

    await sock.sendMessage(chatId, { text, mentions: [who] }, { quoted: msg });
}
