...
const NovaUltra = { command: ['طلاق'], description: 'الطلاق من الشريك', elite: 'off', group: false, prv: false, lock: 'off' };
...
async function execute({ sock, msg, args }) {
    ...
    if (!user.marry) return reply(sock, chatId, '《✧》 أنت لست متزوجاً من أحد.', msg);

    const partnerId   = user.marry;
    const partnerUser = db[partnerId] || {};
    user.marry        = '';
    if (partnerUser) partnerUser.marry = '';
    saveUser(db);

    return reply(sock, chatId,
        `✎ *${user.name || senderJid.split('@')[0]}* تطلق من *${partnerUser.name || partnerId.split('@')[0]}*.`, msg);
}
