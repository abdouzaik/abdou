...
const NovaUltra = { command: ['حذف_وصف'], description: 'حذف وصف الملف الشخصي', elite: 'off', group: false, prv: false, lock: 'off' };
async function execute({ sock, msg, args }) {
    const chatId    = msg.key.remoteJid;
    const senderJid = msg.key.participant || msg.key.remoteJid;
    const { db, user } = getUser(senderJid);
    if (!user.description) return reply(sock, chatId, '《✧》 لم تقم بتعيين وصف بعد.', msg);
    user.description = '';
    saveUser(db);
    return reply(sock, chatId, '✎ تم حذف وصفك بنجاح.', msg);
}
