...
const NovaUltra = { command: ['تعيين_وصف'], description: 'تعيين وصف الملف الشخصي', elite: 'off', group: false, prv: false, lock: 'off' };
async function execute({ sock, msg, args }) {
    const chatId    = msg.key.remoteJid;
    const senderJid = msg.key.participant || msg.key.remoteJid;
    const { db, user } = getUser(senderJid);
    const input = args.join(' ').trim();
    if (!input) return reply(sock, chatId, '《✧》 اكتب وصفاً لملفك.\n> مثال: *تعيين_وصف مرحباً أنا أستخدم واتساب!*', msg);
    user.description = input;
    saveUser(db);
    return reply(sock, chatId, `✎ تم تعيين وصفك بنجاح ✔️`, msg);
}
