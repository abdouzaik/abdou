...
const NovaUltra = { command: ['حذف_ميلاد'], description: 'حذف تاريخ الميلاد', elite: 'off', group: false, prv: false, lock: 'off' };
async function execute({ sock, msg, args }) {
    const chatId    = msg.key.remoteJid;
    const senderJid = msg.key.participant || msg.key.remoteJid;
    const { db, user } = getUser(senderJid);
    if (!user.birth) return reply(sock, chatId, '《✧》 لم تقم بتعيين تاريخ ميلادك بعد.', msg);
    user.birth = '';
    saveUser(db);
    return reply(sock, chatId, '✎ تم حذف تاريخ ميلادك بنجاح.', msg);
}
