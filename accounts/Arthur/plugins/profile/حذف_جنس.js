...
const NovaUltra = { command: ['حذف_جنس'], description: 'حذف الجنس', elite: 'off', group: false, prv: false, lock: 'off' };
async function execute({ sock, msg, args }) {
    const chatId    = msg.key.remoteJid;
    const senderJid = msg.key.participant || msg.key.remoteJid;
    const { db, user } = getUser(senderJid);
    if (!user.genre) return reply(sock, chatId, '《✧》 لم تقم بتعيين جنسك بعد.', msg);
    user.genre = '';
    saveUser(db);
    return reply(sock, chatId, '✎ تم حذف جنسك بنجاح.', msg);
}
