...
export default {{
    ...
    execute: async ({{ sock, msg, args }}) => {{
        ...
        try {{
            ...
            const videoUrl = await getYTVideo(video.url);
            if (!videoUrl) {{ await react(sock, msg, '❌'); return reply(sock, chatId, '❌ فشل التحميل — جرب مرة أخرى', msg); }}
            const videoBuf = await getBuffer(videoUrl);
            await sock.sendMessage(chatId, {{ video: videoBuf, fileName: `${{video.title}}.mp4`, mimetype: 'video/mp4' }}, {{ quoted: msg }});
            await react(sock, msg, '✅');
        }} catch (e) {{
            await react(sock, msg, '❌');
